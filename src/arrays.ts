...
export function tripleNumbers(numbers: number[]): number[] {
    ...
    modNumbers = numbers.map((num: number): number => num * 3);
...
export function stringsToIntegers(numbers: string[]): number[] {
    let nowNum: number[] = [];
    nowNum = numbers.map((string: string): number =>
        +string % 1 != 0 ? 0 : +string,
    );
    return nowNum;
}
...
export const removeDollars = (amounts: string[]): number[] => {
    let dollarAr: string[] = [];
    dollarAr = amounts.map((string: string): string =>
        string[0] == "$" ? string.slice(1) : string,
    );
    let nowNum: number[] = [];
    nowNum = dollarAr.map((string: string): number =>
        +string % 1 != 0 ? 0 : +string,
    );
    return nowNum;
};
...
export const shoutIfExclaiming = (messages: string[]): string[] => {
    return messages
        ...
        .map((msg) => (msg.endsWith("!") ? msg.toLocaleUpperCase() : msg));
};
